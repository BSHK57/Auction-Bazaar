...
// Middleware
app.use(cors({
  origin: ["https://auction-bazaar-24.netlify.app","http://localhost:4200"],
  credentials: true
}));
...
app.post('/create-auction', upload.single('itemImage'), async (req, res) => {
  try {
    ...
    const base64Image = req.file ? req.file.buffer.toString('base64') : '';
...
app.get('/bids-dashboard/:bidderId', async (req, res) => {
  try {
    ...
    participatedBids.reverse().forEach((bid) => {
